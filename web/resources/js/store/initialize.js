...
const actions = {
    async getUserHasProjectAndTodo(context, route) {
        const response = await axios.get("/api/initialize").catch((err) => {
            console.error(err);
        });

        if (response.status === OK) {
            context.commit("project/setProjectList", response.data.project, {
                root: true,
            });
            ...
            if (route.name === "todoList") {
                ...
                if (!response.data.project[projectUuid])
                    context.commit("error/setCode", NOT_FOUND, { root: true });
                context.commit(
                    "project/setProject",
                    response.data.project[projectUuid],
                    { root: true }
                );
                context.commit("todo/selectTodoList", projectUuid, {
                    root: true,
                });
