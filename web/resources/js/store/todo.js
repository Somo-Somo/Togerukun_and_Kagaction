...
const mutations = {
    ...
    addTodoForTodoList(state, newTodo) {
        ...
        for (const [key, todo] of Object.entries(todoList)) {
            ...
            // 追加するTodoと親が同じのTodoの場合または同じ親で追加するTodoより階層が高いとき
                ...
                todoParentOrBrother &&
                (todo.parentUuid === newTodo.parentUuid ||
                    newTodo.depth <= todo.depth)
            ) {
                if (todo.leftSideOfLine[newTodo.depth])
                    todo.leftSideOfLine.splice(newTodo.depth, 1, {
                        lastChild: false,
                    });
                newTodoList.push(todo);
            }
            // 追加するTodoと同じ階層のTodo(兄弟)があるかつ親Todo以上の階層にTodoが戻った場合
                ...
                newTodo["leftSideOfLine"].splice(newTodo.depth, 0, {
                    lastChild: true,
                });
            ...
            if (
                Number(key) + 1 === todoList.length &&
                todoList.length === newTodoList.length
            ) {
                newTodo["leftSideOfLine"].push(...newTodoSpaces);
                newTodo["leftSideOfLine"].splice(newTodo.depth, 0, {
                    lastChild: true,
    ...
    updateAccomplish(state, accomplish) {
    ...
    deleteTodo(state, todo) {
        ...
        for (const [key, value] of Object.entries(todoList)) {
            ...
            deleteTodoChild =
                deleteTodoChild && todo.depth < value.depth ? true : false;
            // 削除しないtodoの場合
        ...
        if (lastChildKey) {
            for (let key = lastChildKey; key < deleteTodoKey; key++) {
                newTodoList[key]["leftSideOfLine"].splice(todo.depth, 1, {
                    lastChild: true,
                });
            }
        }
        ...
        if (newTodoList.length && !lastChildKey)
            newTodoList[parentKey]["child"] = false;
        state.todoList = newTodoList;
...
const actions = {
    ...
    async updateAccomplish(context, todo) {
        context.commit("setTodo", todo);
        context.commit("updateAccomplish", todo.accomplish);
        if (todo.accomplish) {
            const response = await axios.put(
                "/api/todo/" + todo.uuid + "/accomplish"
            );
            if (response.status !== OK) {
